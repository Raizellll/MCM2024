...
def profit_inner(N, S, R, D_final, P_final, I_final, u_final, x_final, y_final, m_semi, D_semi, P_semi, I_semi, u_semi, x_semi, y_semi, m_part, P_part, I_part, l_part, x_part):
    ...
    final_recycle = N * (1 / u_final - 1) * R * (1 - x_final) # 成品回收final_recycle
...
# 定义目标为最大化 profit_case 函数
creator.create("FitnessMax", base.Fitness, weights=(1.0,))
creator.create("Individual", list, fitness=creator.FitnessMax)

toolbox = base.Toolbox()

# 定义变量取值范围，x_part 和 x_semi 是 0-1 列表，x_final 和 y_final 是 0-1 整数
toolbox.register("x_part", random.choices, [0, 1], k=8)
toolbox.register("x_semi", random.choices, [0, 1], k=3)
toolbox.register("x_final", random.randint, 0, 1)
toolbox.register("y_semi", random.choices, [0, 1], k=3)
toolbox.register("y_final", random.randint, 0, 1)

# 创建个体
toolbox.register("individual", tools.initCycle, creator.Individual, 
                 (toolbox.x_part, toolbox.x_semi, toolbox.x_final, toolbox.y_semi, toolbox.y_final))
...
def evaluate(individual):
    x_part, x_semi, x_final, y_semi, y_final = individual
    return profit_case(100, (x_part, x_semi, x_final), (y_semi, y_final)),
...
def custom_mutate(individual, indpb):
    ...
    for i in range(len(individual[1])):
        if random.random() < indpb:
            individual[1][i] = type(individual[1][i])(not individual[1][i])
    
    # 对 x_final 和 y_final 进行单个变异
    if random.random() < indpb:
        individual[2] = type(individual[2])(not individual[2])
    
    for i in range(len(individual[3])):
        if random.random() < indpb:
            individual[3][i] = type(individual[3][i])(not individual[3][i])

    if random.random() < indpb:
        individual[4] = type(individual[4])(not individual[4])
...
def main():
    random.seed(42)
    pop = toolbox.population(n=300)  # 初始种群
...
# 扁平化数据
# 使用列表推导式扁平化列表
flat_solution = [item for sublist in best_solution for item in (sublist if isinstance(sublist, list) else [sublist])]

# print(flat_solution)

# 创建 DataFrame
df = pd.DataFrame([flat_solution], columns=[
...
# 保存到 Excel 文件
df.to_excel('Result4_3.xlsx',sheet_name='最大化每百件商品利润的生产策略', index=False, header=True)
